...
HIDARI_OPEN_ANGLE = -15
HIDARI_CLOSED_ANGLE = 20
MIGI_OPEN_ANGLE = -20
MIGI_CLOSED_ANGLE = 10
...
def run_auto_test(servo1, servo2, poll_interval: float, retry_count: int = 0) -> None:
    ...
    try:
        ...
        move_servo(servo1, MIGI_OPEN_ANGLE)
        move_servo(servo2, HIDARI_OPEN_ANGLE)
        ...
        if GPIO.input(LIMIT1_PIN) == GPIO.LOW:  # もし右側のリミットスイッチにモーターが触れていたなら
            ...
            # サーボを閉じる
            move_servo(servo2, HIDARI_CLOSED_ANGLE)
            move_servo(servo1, MIGI_OPEN_ANGLE)
            time.sleep(0.5)

            # 雑巾保管場所を下げる(下げる時間はまた後で設定)
            if not _send_payload_for([0, 0, MOTOR_SPEED, 0, 0, 0, 1, 1], 0.5, poll_interval):
                return
            # 装填機構を左にスライド
            ...
            if not move_until_limit([0, 0, 0, MOTOR_SPEED, 0, 0, 1, 1], LIMIT3_PIN, poll_interval):
                return
            # サーボを閉じる
            move_servo(servo1, MIGI_CLOSED_ANGLE)
            move_servo(servo2, HIDARI_OPEN_ANGLE)
